const CACHE_NAME = 'ORBITXD_CACHE-v1';
self.addEventListener('install', function(){
    // Guardar archivos iniciales

    caches.open(CACHE_NAME).then(function(cache){
        cache.addAll(['/index.html','/dist/javascript/bundle.js','/public/images/10.jpg']);
...
function searchInCacheOrMakeRequest(request){
   const cachePromise =  caches.open(CACHE_NAME);
    ...
    return Promise.all([cachePromise,matchPromise]).then(function([cache,cacheResponse]){
      ...
      const fecthPromise =  fetch(request).then(function(fetchResponse){
        ...
        return cacheResponse || fecthPromise;
